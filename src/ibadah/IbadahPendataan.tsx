...
import { getMukholif, addMukholif, deleteMukholif, deleteAllMukholif, Mukholif } from '../lib/api';
...
const JENIS_UTAMA = ['Mashol', 'Masbuk', 'Lainnya'];

const SUB_LAINNYA = [
  'Senderan ke Tiang', 'Tidak Pakai celana Syari', 'Ribut di Masjid',
  'Tidak Pakai Songkok/Peci', 'Lari Di Masjid', 'bercanda Di Masjid',
   'Bercanda Saat Sholat', 'Ngobrol',
];

export function IbadahPendataan() {
  ...
  const [jenisUtama, setJenisUtama] = useState('Mashol');
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await addMukholif({ nama, kelas, jenis, catatan, date });
      setSaved(true);
      await loadData();
  ...
  const resetForm = () => { setNama(''); setKelas('Kelas 9'); setJenisUtama('Mashol'); setSubJenis(''); setCatatan(''); setDate(new Date().toISOString().split('T')[0]); };

  const handleDelete = async (id: string) => {
    if (!confirm('Hapus data ini?')) return;
    await deleteMukholif(id);
    await loadData();
  };

  const handleDeleteAll = async () => {
    setDeletingAll(true);
    try { await deleteAllMukholif(); await loadData(); setShowDeleteAll(false); } finally { setDeletingAll(false); }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-400">Kelola data pelanggaran ibadah</p>
        </div>
        <div className="flex gap-2">
          {data.length > 0 && (
            ...
            >
              <Trash2 className="w-4 h-4" /> Hapus Semua Mukholif
            </button>
          )}
          <button
          ...
          >
            <Plus className="w-4 h-4" /> Tambah Pelanggar
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-fadeIn overflow-y-auto max-h-[90vh]">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h2 className="text-lg font-bold text-black">Tambah Pelanggar</h2>
                ...
                <div className="flex gap-2 mb-3">
                  {JENIS_UTAMA.map(j => (
                    <button type="button" key={j}
                      ...
                      className={`flex-1 py-2.5 rounded-xl border-2 text-sm font-semibold transition-all ${jenisUtama === j ? 'bg-black text-white border-black' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}>
                      {j}
                    </button>
                  ))}
                </div>
                {/* Sub jenis – hanya untuk Lainnya */}
                {jenisUtama === 'Lainnya' && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-fadeIn">
            ...
            <p className="text-sm text-gray-600 mb-5">
              Anda akan menghapus <strong>{data.length} data mukholif</strong> secara permanen.
            </p>
