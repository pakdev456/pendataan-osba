...
import { QismDashboard } from './QismDashboard';
import { QismPendataan } from './QismPendataan';
import { QismLaporan } from './QismLaporan';
...
export function QismLayout({ config }: Props) {
  ...
  const renderPage = () => {
    switch (page) {
      case 'dashboard': return <QismDashboard config={config} />;
      case 'pendataan': return <QismPendataan config={config} />;
      case 'laporan': return <QismLaporan config={config} />;
      default: return <QismDashboard config={config} />;
    }
  };

  return (
    <div className="min-h-screen bg-gray-50 flex">
      {loading && <MoonLoader title={`${config.title} OSBA`} subtitle="Berpindah halaman" />}
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <OsbaLogo size={40} />
              <div>
                <p className="text-sm font-bold text-black leading-tight">{config.title}</p>
      ...
      <main className="flex-1 lg:ml-64 min-h-screen flex flex-col">
        <header className="lg:hidden sticky top-0 bg-white border-b border-gray-200 z-30 flex items-center gap-3 px-4 py-4">
          ...
          <span className="font-semibold text-black">{config.title} OSBA</span>
        </header>
